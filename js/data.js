...
// 教材を何問ずつのまとまりに区切るか（2026-08-05に10問から20問へ変更）
const SET_SIZE = 20;

// 「復習コーナー」「苦手な問題」で一度に出す問題数。
// 区切りを20問にしても、ここは10問のまま据え置きます（1回が長くなりすぎるため）。
const REVIEW_SIZE = 10;

/* ============================================================
   問題に付ける画像（2026-08-10 追加）
   ------------------------------------------------------------
   CSVの「画像」欄に画像を指定できます。書き方は2通りです。

     ・ファイル名だけ（例：chizu-01.jpg）
         → questions/images/ に置いた画像を表示します。おすすめはこちら。
           差し替えはファイルを置き換えるだけで済み、オフラインでも出ます。
     ・https:// で始まるアドレス
         → その場所の画像を表示します。
           通信がないと出ず、他人のサイトの画像は著作権の問題があります。

   空欄なら、これまでどおり画像なしで出題します。
   ============================================================ */
const IMAGE_DIR = './questions/images/';

function imageSrcOf(item) {
  const raw = String((item && item.image) || '').trim();
  if (!raw) return '';
  if (/^https?:\/\//i.test(raw)) return raw;
  /*
   * ファイル名だけを受け付けます。
   * 「../」やフォルダ区切りが書かれていると画像置き場の外を指せてしまうため、
   * その場合は画像なしとして扱います。
   */
  if (/[\\/]/.test(raw) || raw.indexOf('..') >= 0) return '';
  return IMAGE_DIR + encodeURIComponent(raw);
}
